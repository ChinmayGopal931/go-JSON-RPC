...
contract Fixtures is Deployers, DeployPermit2 {
    ...
    function seedBalance(address to) internal {
        MOCKERC20PERMIT(Currency.unwrap(currency0)).transfer(to, STARTING_USER_BALANCE);
        MOCKERC20PERMIT(Currency.unwrap(currency1)).transfer(to, STARTING_USER_BALANCE);
    }
    ...
    function approvePosmCurrency(Currency currency) internal {
        ...
        MOCKERC20PERMIT(Currency.unwrap(currency)).approve(address(permit2), type(uint256).max);
